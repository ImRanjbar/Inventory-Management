...
#include "ui_sale_invoice_window.h"
#include "../Currency.h"
#include "../datahandler.h"
#include <QFileDialog>
#include <QStyledItemDelegate>

sale_invoice_window::sale_invoice_window(Invoice& invoice,QWidget *parent) :
    QDialog(parent),
    ui(new Ui::sale_invoice_window)
{
    ui->setupUi(this);
    m_invoice = invoice;
...
sale_invoice_window::~sale_invoice_window()
{
    delete ui;
}

void sale_invoice_window::setLabels(){
    ...
    ui->LB_manufacturerName->setText(QString::fromStdString(m_invoice.getCustomerName()));
    ui->LB_MID->setText(QString::fromStdString(m_invoice.getCustomerID()));
    ui->LB_amount->setText(QString::fromStdString(Currency::currencySymbol) + QString::number(m_invoice.getTotalAmount()));
}

void sale_invoice_window::initializeTableView(){
...
void sale_invoice_window::setComboBox(){
...
void sale_invoice_window::addItems(){
...
void sale_invoice_window::search(const QString &text){
...
void sale_invoice_window::updateTableViewWithSearchCriteria(const QString &text, MemberFunction memberFunction){
    const std::vector<InvoiceItem>& items = m_invoice.getInvoiceItemModel().getItems();
...
void sale_invoice_window::on_TV_items_doubleClicked(const QModelIndex &index){
...
void sale_invoice_window::on_PB_print_clicked(){
...
void sale_invoice_window::on_LE_search_textChanged(const QString &arg1){
    search(arg1);
}
